...
@/Users/majskiy/Documents/projects/coter/coter-terraform/.cursor/gsd-core/references/gsd-run-resolver.md
...
- `--text` forces text-mode choices for runtimes without `conversational prompting`.
...
Set:
- `TEXT_MODE=true` if `--text` is present or `text_mode` is true. When `TEXT_MODE` is active, replace every `conversational prompting` call below with a plain-text numbered list and ask the user to type their choice number — required for non-Claude runtimes (OpenAI Codex, Gemini CLI, etc.) where `conversational prompting` is not available.
