...
for cand in \
  ...
  "/Users/majskiy/Documents/projects/coter/coter-terraform/.cursor/gsd-core/bin/gsd-tools.cjs"; do
...
The `GSD_DIR` value emitted by `get_installed_version` (line 4) resolves to the runtime-specific config dir (`/Users/majskiy/Documents/projects/coter/coter-terraform/.cursor/`, `~/.gemini/`, `~/.codex/`, etc.), so the script invocation works for every runtime — not just Claude. If `GSD_DIR` is empty (scope `UNKNOWN`), skip this step and go directly to install.
...
global: `/Users/majskiy/Documents/projects/coter/coter-terraform/.cursor/`, `~/.config/opencode/`, `~/.opencode/`, `~/.gemini/`, `~/.config/kilo/`, or `~/.codex/`
...
**If `RESTORE_ELIGIBLE` > 0:** ask with `conversational prompting`:
...
**Text mode** (`--text`, or a runtime without `conversational prompting`): present the
