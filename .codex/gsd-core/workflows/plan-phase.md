...
@/Users/majskiy/Documents/projects/coter/coter-terraform/.codex/gsd-core/references/ui-brand.md
@/Users/majskiy/Documents/projects/coter/coter-terraform/.codex/gsd-core/references/revision-loop.md
@/Users/majskiy/Documents/projects/coter/coter-terraform/.codex/gsd-core/references/gate-prompts.md
...
PRD_PARAM=""; if [[ "{{GSD_ARGS}}" =~ (^|[[:space:]])--prd[[:space:]]+([^[:space:]-][^[:space:]]*) ]]; then PRD_PARAM="--prd ${BASH_REMATCH[2]}"; fi
INGEST_PARAM=""; if [[ "{{GSD_ARGS}}" =~ (^|[[:space:]])--ingest[[:space:]]+([^[:space:]-][^[:space:]]*) ]]; then INGEST_PARAM="--ingest ${BASH_REMATCH[2]}"; fi
RESEARCH_PHASE_PARAM=""; if [[ "{{GSD_ARGS}}" =~ (^|[[:space:]])--research-phase[[:space:]]+([^[:space:]-][^[:space:]]*) ]]; then RESEARCH_PHASE_PARAM="--research-phase ${BASH_REMATCH[2]}"; fi
REVIEWS_PARAM=""; if [[ "{{GSD_ARGS}}" =~ (^|[[:space:]])--reviews([[:space:]]|$) ]]; then REVIEWS_PARAM="--reviews"; fi
CHUNKED_PARAM=""; if [[ "{{GSD_ARGS}}" =~ (^|[[:space:]])--chunked([[:space:]]|$) ]]; then CHUNKED_PARAM="--chunked"; fi
...
GAPS_MODE=false
if [[ "{{GSD_ARGS}}" =~ (^|[[:space:]])--gaps([[:space:]]|$) ]]; then GAPS_MODE=true; fi
GAPS_EXEC_FLAG=""
...
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `/Users/majskiy/Documents/projects/coter/coter-terraform/.codex/gsd-core/references/skeleton-template.md` — the planner reads it when producing SKELETON.md (lazy; not loaded on non-skeleton runs).
...
**If found:**
1. Read template: `/Users/majskiy/Documents/projects/coter/coter-terraform/.codex/gsd-core/templates/VALIDATION.md`
...
**Read and run** the gate + edge probe in `/Users/majskiy/Documents/projects/coter/coter-terraform/.codex/gsd-core/references/specless-probe-fallback.md`
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `/Users/majskiy/Documents/projects/coter/coter-terraform/.codex/gsd-core/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
**WALKING_SKELETON:** ${WALKING_SKELETON} (when true, the first deliverable must be a Walking Skeleton — Read the template at `/Users/majskiy/Documents/projects/coter/coter-terraform/.codex/gsd-core/references/skeleton-template.md` and produce SKELETON.md alongside PLAN.md.)
**Granularity:** {granularity}

${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Read `/Users/majskiy/Documents/projects/coter/coter-terraform/.codex/gsd-core/references/planner-mvp-mode.md` now and follow its vertical-slice planning rules. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
...
else-branch below, per §A/§B/§C of `/Users/majskiy/Documents/projects/coter/coter-terraform/.codex/gsd-core/references/specless-probe-fallback.md`
